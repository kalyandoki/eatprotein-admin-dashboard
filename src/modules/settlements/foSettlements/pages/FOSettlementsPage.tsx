// src/modules/settlements/foSettlements/pages/FOSettlementsPage.tsx
...
import AddFOSettlementModal from "../components/AddFOSettlementModal";
import EditFOSettlementModal from "../components/EditFOSettlementModal";
import DeleteFOSettlementModal from "../components/DeleteFOSettlementModal";
import Pagination from "../../../../components/common/Pagination";

export default function FOSettlementsPage() {
  const dispatch = useAppDispatch();
  const { settlements, status } = useAppSelector(
    (state) => state.foSettlements
  );
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState<string>("all");
  const [dateFilter, setDateFilter] = useState<string>("");
  const [selectedSettlement, setSelectedSettlement] =
    useState<FOSettlement | null>(null);
  ...
  useEffect(() => {
    if (status === "idle") dispatch(fetchFOSettlements());
  }, [status, dispatch]);

  // Filter settlements based on search and filters
  const filteredSettlements = settlements.filter((settlement) => {
    const matchesSearch =
      settlement.foName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleRefresh = () => {
    dispatch(fetchFOSettlements());
  };

  const handleEdit = (settlement: FOSettlement) => {
    setSelectedSettlement(settlement);
    setShowEditModal(true);
  };

  const handleDelete = (settlement: FOSettlement) => {
    setSelectedSettlement(settlement);
  ...
  const handleExport = () => {
    ...
    console.log("Exporting FO settlements...");
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div className="mb-4 md:mb-0">
              ...
              <p className="text-emerald-100 text-lg">
                Manage and track franchise owner settlements
              </p>
            </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search by FO Name or Settlement ID"
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
              <h2 className="text-lg font-semibold text-gray-800">
                FO Settlements ({filteredSettlements.length})
              </h2>
            </div>
                  ...
                  {currentSettlements.length === 0 ? (
                    ...
                    currentSettlements.map((settlement) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">
                            {formatCurrency(settlement.totalBalAppAmount)}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">
                            {formatCurrency(settlement.totalFoAmount)}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">
        ...
        {/* Modals */}
        {showAddModal && (
          <AddFOSettlementModal onClose={() => setShowAddModal(false)} />
        )}
        {showEditModal && selectedSettlement && (
          <EditFOSettlementModal
