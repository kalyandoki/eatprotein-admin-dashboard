// src/modules/settlements/foSettlements/components/EditFOSettlementModal.tsx
import React, { useState, useEffect } from "react";
import { useAppDispatch } from "../../../../store/hooks";
import { editFOSettlement, FOSettlement } from "../foSettlementsSlice";
import { FiX, FiSave, FiCalendar, FiDollarSign } from "react-icons/fi";

export default function EditFOSettlementModal({
  settlement,
  onClose,
}: {
  settlement: FOSettlement;
  onClose: () => void;
}) {
  ...
  const [form, setForm] = useState({
    ...
    foName: settlement.foName,
    totalStores: settlement.totalStores,
    totalAmount: settlement.totalAmount,
    totalBalAppAmount: settlement.totalBalAppAmount,
    totalFoAmount: settlement.totalFoAmount,
  ...
  const validateForm = () => {
    ...
    if (form.totalBalAppAmount < 0) {
      newErrors.totalBalAppAmount =
        "Total Bal App Amount must be greater than or equal to 0";
      isValid = false;
    }

    if (form.totalFoAmount < 0) {
      newErrors.totalFoAmount =
        "Total Fo Amount must be greater than or equal to 0";
      isValid = false;
    }
  ...
  const handleSave = () => {
    ...
    dispatch(editFOSettlement(form));
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden transform transition-all">
        <div className="bg-gradient-to-r from-emerald-600 to-teal-700 text-white px-6 py-4 rounded-t-xl flex justify-between items-center">
          <h2 className="text-xl font-semibold">Edit FO Settlement</h2>
          <button
            onClick={handleClose}
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-70px)]">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        totalBalAppAmount: parseFloat(e.target.value) || 0,
                      })
                    }
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        totalFoAmount: parseFloat(e.target.value) || 0,
                      })
                    }
                    placeholder="Enter total FO amount"
                  />
                </div>
                {errors.totalFoAmount && (
                  <p className="mt-1 text-sm text-red-600">
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-600">Total Bal App Amount</p>
                <div className="text-lg font-semibold text-gray-800">
                  {formatCurrency(form.totalBalAppAmount)}
                </div>
              </div>
              <div>
                <p className="text-sm text-gray-600">Total Fo Amount</p>
                <div className="text-lg font-semibold text-gray-800">
                  {formatCurrency(form.totalFoAmount)}
                </div>
              </div>
            </div>
          </div>
