...
import { showConfirmationModal } from "../../../store/slices/uiSlice"; // Import the new action
import {
  FiPlus,
  FiSearch,
  FiRefreshCw,
...
export default function LocationsPage() {
  ...
  // MODIFIED: This function now shows the confirmation modal
  const handleToggleStatus = (location: Location) => {
    ...
    dispatch(
      showConfirmationModal({
        ...
        message: `Are you sure you want to ${actionText} the location "${location.areaName}"?`,
        confirmButtonText: `Yes, ${actionText}`,
        // The onConfirm callback dispatches the actual status change action
        onConfirm: () => {
          dispatch(toggleLocationStatus(location.id));
        },
      })
    );
  };
                    ...
                    currentLocations.map((location) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <button
                              // MODIFIED: onClick now calls the new handler and passes the full location object
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          ...
                          <button
                            // UNCHANGED: This still calls your original delete handler
                            onClick={() => handleDelete(location)}
                            className="text-red-600 hover:text-red-900 transition-colors duration-150 p-1 rounded-full hover:bg-red-50"
                            title="Delete"
                          >
