// src/modules/users/storeAdmins/pages/StoreAdminsPage.tsx
...
import AddStoreAdminModal from "../components/AddStoreAdminModal";
import EditStoreAdminModal from "../components/EditStoreAdminModal";
import DeleteStoreAdminModal from "../components/DeleteStoreAdminModal";
import Pagination from "../../../../components/common/Pagination";

export default function StoreAdminsPage() {
  const dispatch = useAppDispatch();
  const { admins, status } = useAppSelector((state) => state.storeAdmins);
  ...
  useEffect(() => {
    if (status === "idle") dispatch(fetchStoreAdmins());
  }, [status, dispatch]);

  // Filter admins based on search and filters
  const filteredAdmins = admins.filter((admin) => {
    const matchesPhone = admin.contactNo.includes(searchPhone);
    const matchesLocation = admin.city
      .toLowerCase()
  ...
  const handleEdit = (admin: StoreAdmin) => {
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div className="mb-4 md:mb-0">
              ...
              <p className="text-emerald-100 text-lg">
                Manage store admins and their information
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-blue-500 transform transition-all duration-200 hover:shadow-lg hover:-translate-y-1">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500 text-sm font-medium">Rebala</p>
                <p className="text-3xl font-bold text-gray-800 mt-1">
                  {admins.filter((a) => a.city === "rebala").length}
                </p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-purple-500 transform transition-all duration-200 hover:shadow-lg hover:-translate-y-1">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500 text-sm font-medium">Not Updated</p>
                <p className="text-3xl font-bold text-gray-800 mt-1">
                  {admins.filter((a) => a.city === "Not Updated").length}
                </p>
              </div>
        ...
        {status === "loading" ? (
          <div className="flex justify-center items-center py-20 bg-white rounded-xl shadow-lg">
            <div className="flex flex-col items-center">
              ...
              <p className="mt-4 text-gray-600">Loading store admins...</p>
            </div>
          </div>
        ) : (
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
              <h2 className="text-lg font-semibold text-gray-800">
                Store Admins ({filteredAdmins.length})
              </h2>
            </div>
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          <p className="mt-1 text-sm text-gray-500 max-w-md mx-auto">
                            {searchPhone || searchLocation
                              ...
                              : filteredAdmins.length === 0
                              ? "Get started by adding a new store admin"
        ...
        {/* Modals */}
        {showAddModal && (
          <AddStoreAdminModal onClose={() => setShowAddModal(false)} />
        )}
